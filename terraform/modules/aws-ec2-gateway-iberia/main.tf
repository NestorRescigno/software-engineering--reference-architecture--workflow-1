...
resource "aws_route_table_association" "route_table_association_private" {
  ...
  route_table_id = aws_route_table.private[index(data.aws_subnets.snet_amber_eu_central_1_subnets.ids, each.value.id)].id
}
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  ...
  route_table_id  = aws_route_table.private[index(data.aws_availability_zone.all,  each.value.name)].id
}
